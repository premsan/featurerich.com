...
public interface AuthorityRepository
    ...
    @Query(
            "SELECT a.id, a.version, a.name, a.updated_at, a.updated_by FROM security_authority a INNER"
                    + " JOIN security_role_authority ra ON a.id = ra.authority_id INNER JOIN security_role r ON"
                    + " ra.role_id = r.id INNER JOIN security_user_role ur ON ur.role_id = r.id WHERE"
                    + " ur.user_id = :userId")
